...
public class PrivacyManager {
    ...
    static {
        ...
        for (Hook hook : listHook) {
            ...
            if (restrictionName == null)
                ...
                if (hook.isAvailable())
                    LunaLog.d("Not found restriction=" + restrictionName + " hook=" + hook);
        ...
        try {
            ...
            if (!cached) {
                ...
                if (result.time > 0) {
                    ...
                    LunaLog.d("Caching " + result + " until " + new Date(result.time));
                }
                synchronized (mRestrictionCache) {
                    if (mRestrictionCache.containsKey(key))
    ...
    public static boolean getRestrictionExtra(final XHook hook, int uid, String restrictionName, String methodName,
        ...
        // Check secret
        if (secret == null) {
            LunaLog.e("Secret missing restriction=" + restrictionName + "/" + methodName);
            secret = "";
        }

        // Check restriction
        if (restrictionName == null || restrictionName.equals("")) {
            LunaLog.e("restriction empty method=" + methodName);
        ...
        if (methodName == null || methodName.equals("")) {
            LunaLog.e("Method empty");

        } else if (getHook(restrictionName, methodName) == null) {
            LunaLog.e("Unknown method=" + methodName);
    ...
    public static void applyTemplate(int uid, String templateName, String restrictionName, boolean methods,
        ...
        // Apply template
        LunaLog.d("Applying template=" + templateName);
                        ...
                        for (String permission : listPermission)
                            if (rPermission.toLowerCase().contains(permission.toLowerCase())) {
                                ...
                                if (!aPermission.equals(rPermission))
                                    LunaLog.d("Check permission=" + permission + "/" + rPermission);
                                return true;
                            }
            }
